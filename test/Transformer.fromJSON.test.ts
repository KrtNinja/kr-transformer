...
import { Schema, Transformer } from '../src'
...
describe('Transformer.fromJSON', () => {
  ...
  it(`should not transform value from json, if it can't be cast to type declared in target class, and strict is false`, () => {
    class Bar {}
    ...
    assert.equal(result.bar instanceof Bar, true)
  })

  it('should not throw if json is valid, strict is "true" and descriptor nullable is "true"', () => {
    class Bar {}
  ...
  it('should use value from instance if their type is not declared, strict is "false"', () => {
    class Foo {
      text: string | null = null
    }
  ...
  it('should use instance value when transform array elements into Set and value in json is not array and "strict" = false', () => {
    class Foo {
      set = new Set<string>()
    }
